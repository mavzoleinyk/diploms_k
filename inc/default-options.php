...
function lucida_header_highlight_content_types() {
	$options = array(
		'demo'     => esc_html__( 'Demo', 'lucida' ),
		'page'     => esc_html__( 'Page', 'lucida' ),
	);
...
function lucida_get_content() {
	...
	return sprintf( _x( 'Copyright &copy; %1$s %2$s. All Rights Reserved. %3$s', '1: Year, 2: Site Title with home URL, 3: Privacy Policy Link', 'lucida' ), date_i18n( __( 'Y', 'lucida' ) ), '<a href="'. esc_url( home_url( '/' ) ) .'">'. esc_attr( get_bloginfo( 'name', 'display' ) ) . '</a>', get_the_privacy_policy_link() ) . ' &#124; ' . esc_html( $theme_data->get( 'Name') ) . '&nbsp;' . esc_html__( 'by', 'lucida' ). '&nbsp;<a target="_blank" href="'. $theme_data->get( 'AuthorURI' ) .'">'. esc_html( $theme_data->get( 'Author' ) ) .'</a>';
}
