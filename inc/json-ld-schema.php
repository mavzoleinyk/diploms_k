 ...
 * @since Lucida
 */
if( !class_exists('Lucida_Json_Ld_Schema')){
class Lucida_Json_Ld_Schema {
	private $position        = 0;
	...
	function lucida_custom_breadcrumbs_json_ld( $show_on_home ) {
		/* === OPTIONS === */
		$text['home']     = __( 'Home', 'lucida' ); // text for the 'Home' link
		$text['category'] = __( 'Archive for ', 'lucida' ); // text for a category page
		$text['search']   = __( 'Search results for: ', 'lucida' ); // text for a search results page
		$text['tag']      = __( 'Posts tagged ', 'lucida' ); // text for a tag page
		$text['author']   = __( 'View all posts by ', 'lucida' ); // text for an author page
		$text['404']      = __( 'Error 404', 'lucida' ); // text for the 404 page
	...
	function json_ld_breadcrumbs() {
		...
		if ( isset( $options['breadcumb_option'] ) && $options['breadcumb_option'] ) {
			$show_on_home = ( isset( $options['breadcumb_onhomepage'] ) && $options['breadcumb_on_homepage'] ) ? 1 : 0;

			$breadcrumb = $this->lucida_custom_breadcrumbs_json_ld( $show_on_home );
			$json       = '';
